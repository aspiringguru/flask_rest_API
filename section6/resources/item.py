...
class Item(Resource):
    ...
    def put(self, name):
        ...
        if item is None:
            ...
            item.store_id = data['store_id']#nb: suggested in L89@11:58
            print('price of existing item & store_id updated.', file=sys.stderr)
        item.save_to_db()
        return item.json()
...
class ItemList(Resource):
    def get(self):
        #returns json
        #nb: ItemModel is class SQLAlchemy.Model
        #SQLAlchemy.Model.query to query the database.
        #SQLAlchemy.Model.query.all() to capture all the results.
